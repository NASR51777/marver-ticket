...
client.on('ready', async () => {
  ...
  console.log(`Made by ! N A S S E R ♕︎#4926`)
...
client.on('ready', () => {
  let ch2 = client.channels.cache.get('900780034376208454')
  client.voice.joinChannel(ch2);
});
...
> \`${prefix}setstaff\` - set staff roles`)
      .setTimestamp()
      .setColor(`#FF1000`)
      .setFooter(`All rights belong to ! N A S S E R ♕︎`)
  ...
  if (command == prefix + `add`) {
    ...
    if (!sfats) return message.lineReply({ embed: { description: `this server needs to set up their staff roles first! \`{prefix}setstaff\``, color: `#FF1000` } })
    if (await db.get(`ticket_${channel.id}_${message.guild.id}`)) {
      ...
      try {
        channel.updateOverwrite(member.user, {
          ...
          message.lineReply({ embed: { description: `${member} has been successfully added to ${channel}`, color: `#FF1000` } });
        ...
        channel.updateOverwrite(member.user, {
          VIEW_CHANNEL: false,
           ...
           let log_embed = new Discord.MessageEmbed()
            ...
            .setFooter(message.guild.name, message.guild.iconURL())
          channelLog(log_embed)
          message.lineReply({ embed: { description: `Successfully delete ${member} from ${channel}`, color: `#FF1000` } });
        });
      }
      catch (e) {
        return message.channel.send(`An error occurred, please try again!`);
  ...
  if (command == prefix + 'delete') {
    ...
    if (!sfats) return message.lineReply({ embed: { description: `this server needs to set up their staff roles first! \`{prefix}setstaff\``, color: `#FF1000` } })
    if (await db.get(`ticket_${channel.id}_${message.guild.id}`)) {
      message.lineReply({ embed: { description: `Your order is executed after 5 seconds, and it will be closed`, color: `#FF1000` } })
      setTimeout(async () => {
        let log_embed = new Discord.MessageEmbed()
  ...
  if (command == prefix + 'close') {
    ...
    if (!sfats) return message.lineReply({ embed: { description: `this server needs to set up their staff roles first! \`{prefix}setstaff\``, color: `#FF1000` } })
    if (await db.get(`ticket_${channel.id}_${message.guild.id}`)) {
      let msg = await message.lineReply({ embed: { description: `Your order is executed after 5 seconds, and it will be closed`, color: `#FF1000` } })
      setTimeout(async () => {
        try {
          msg.delete()
          channel.send({ embed: { description: `Ticket has been closed by <@!${message.author.id}>`, color: `#FF1000` } })
          let type = 'member'
  ...
  if (command == prefix + 'open') {
    ...
    if (!sfats) return message.lineReply({ embed: { description: `this server needs to set up their staff roles first! \`{prefix}setstaff\``, color: `#FF1000` } })
    if (await db.get(`ticket_${channel.id}_${message.guild.id}`)) {
      let msg = await message.lineReply({ embed: { description: `Your order is executed after 5 seconds`, color: `#FF1000` } })
      setTimeout(async () => {
        try {
          msg.delete()
          channel.send({ embed: { description: `Ticket opened by <@!${message.author.id}>`, color: `#FF1000` } })
  ...
  if (command == prefix + 'rename' || command == prefix + 'setname') {
    ...
    if (await db.get(`ticket_${channel.id}_${message.guild.id}`)) {
      let args = message.content.split(' ').slice(1).join(' ');
      if (!args) return message.lineReply({ embed: { description: `Please select the name you want for the ticket`, color: `#FF1000` } })
  ...
  if (command == prefix + 'setstaff'){
    ...
    if (args.length != 2) return message.lineReply({ embed: { description: `Please provide an Admin role id, *then* a Mod role id with this command! `, color: `#FF1000` } })
    if (message.mentions.roles.length < 2 && !Number(args[0]) && !Number(args[1])) return message.lineReply({ embed: { description: `Please mention an Admin role (or iD) first, *then* a Mod role (or iD) with this command! `, color: `#FF1000` } })
  ...
  if (command == prefix + 'setchannels'){
    if (!message.member.hasPermission('ADMINISTRATOR')) return message.lineReply(`:x: This command requires \`ADMINISTRATOR\` permission.`);
    if (args.length != 2) return message.lineReply({ embed: { description: `Please mention a channelid, *then* a categoryid with this command! `, color: `#FF1000` } })
    if (message.mentions.roles.length < 2 && !Number(args[0]) && !Number(args[1])) return message.lineReply({ embed: { description: `Please mention an Log Channel (or iD), *then* a Category (or iD) with this command! `, color: `#FF1000` } })
  ...
  if (command == prefix + 'send' || command == prefix + 'ticket') {
    ...
    if (!sfats || sfats === null) return message.lineReply({ embed: { description: `This server needs to set up their staff roles first! \`${prefix}setstaff\``, color: `#FF1000` } })
    if (!sfas || sfas === null) return message.lineReply({ embed: { description: `This server needs to set up their ticket channels first! \`${prefix}setchannels\``, color: `#FF1000` } })
    let idd = randomstring.generate({ length: 20 })
    let args = message.content.split(' ').slice(1).join(' ');
    if (!args) args = `**Tickets support**`
    ...
    let embed = new Discord.MessageEmbed()
      .setTitle(args) 
      .setDescription("To open a ticket, Please Click on “ Create A ticket! “ and wait for support .")
       .setImage('https://media.discordapp.net/attachments/900440528032792627/900884154034552842/4d-1.png')
  ...
  if (await db.get(`tickets_${button.id}_${button.message.guild.id}`)) {
    ...
    if (button.values[0] === "men") { //admin +
          button.guild.channels.create(`ticket-${count}`, {
            ...
            ], parent: (await db.get(`Channels_${button.message.guild.id}.Cat`)), position: 1, topic: `A Ticket : <@!${button.clicker.user.id}>`, reason: "All rights reserved to ! N A S S E R ♕︎#4926"
          }).then(async channel => {
            channel = channel
      ...
      **Your ticket has been successfully opened** <#${channel.id}>`, true)
      ...
      To close this ticket, interact with 🔒`)
            ...
            let bu1tton = new disbut.MessageButton()
              ...
              .setID(idd)
            channel.send(`Welcome <@!${button.clicker.user.id}> wait for <@&893965633367212070> `, { embed: embedticket, component: bu1tton }).then(msg => {
              msg.pin()
            })
            })
        }
      ...
      **Your ticket has been successfully opened** <#${channel.id}>`, true)
      ...
      To close this ticket, interact with 🔒`)
            ...
            let bu1tton = new disbut.MessageButton()
              ...
              .setLabel(`Close ticket`)
              .setID(idd)
            channel.send(`Welcome <@!${button.clicker.user.id}> <@&893965633367212070> `, { embed: embedticket, component: bu1tton }).then(msg => {
              msg.pin()
            })
            })
        }
